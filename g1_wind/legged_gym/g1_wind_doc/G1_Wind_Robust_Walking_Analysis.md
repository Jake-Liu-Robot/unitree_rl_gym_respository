...
**Curriculum 自动升级逻辑（已实现）**：窗口累积式（upgrade_window=300 次 reset），混合升级（80% envs 升级, 20% 留级防灾难性遗忘）。升级条件：survival>0.7 AND tracking>0.4。降级条件（50% envs）：survival<0.4 AND tracking<0.3。升/降级后 500 resets 冷却期。

### 3.5 Phase 4：训练与实验（第 4-6 周）

**实验计划与结果：**

| 实验 | 目的 | 配置 | L5 存活率 | 失败数 | 状态 |
|------|------|------|----------|--------|------|
| Exp 1: Baseline | 无风训练的基准策略 | push_robots=False, wind=False | 5% | 45/84 | **Done** |
| Exp 2: Push Only | 传统瞬时推力鲁棒策略 | push_robots=True, wind=False | 81% | 16/84 | **Done** |
| Exp 3: Wind-Trained | 风力训练的策略（完整方法） | wind=True, curriculum=True | **100%** | **0/84** | **Done** |
| Exp 4: No Curriculum | 无 curriculum 直接 L3 风训练 | wind=True, curriculum=False | 48% | 18/84 | **Done** |
| Exp 5: No Wind Reward | 去掉风力专用 reward | wind=True, 只保留基础 reward | 99% | 0/84 | **Done** |

**消融结论（贡献度排序）：**
1. 风模型训练 (Exp1→Exp3): 最关键，L5 5%→100%
2. 课程学习 (Exp4→Exp3): 核心贡献，L5 48%→100%
3. 风物理 vs Push (Exp2→Exp3): 显著改进，L5 81%→100%
4. 风奖励 (Exp5→Exp3): 边际改进，L5 trk 0.272→0.254
...
Suite F: 指令变化
  ├── F1: 站立（vx=0, vy=0）
  ├── F2-F4: 慢走/正常/快走（vx=0.3/0.6/1.0）
  ├── F5: 侧走（vy=0.3）
  ├── F6: 转弯（yaw=0.5）
  └── F7-F8: 逆风/顺风行走
```

每实验 84 个场景（Suite A: 6, B-F 各子项在 L3/L4/L5 测试）。
...
5. legged_gym/g1_wind_test/eval_wind_robustness.py  ✓
   - 6 维测试矩阵：level × mode × direction × OU extreme × OOD pattern × command
   - A/B policy 对比功能
   - 84 场景全覆盖评估，JSON 输出
```
...
**层面二 — 方法论的创新**：提出基于 OU 过程的三层风力建模方法（基风+湍流+阵风），v3.2 per-body 气动力物理模型（相对速度、3D 椭球投影、高度剖面、压力中心），以及 6 级 curriculum 学习策略。消融实验证明课程学习是核心贡献（L5 存活率从 48% 提升至 100%），而风奖励提供的边际改进较小（99%→100%）。

**层面三 — 评估标准的创新**：建立 6 维测试矩阵（84 场景 × 5 实验 = 420 次评估），覆盖风速级别、风模式分解、风向、OU 参数极端值、分布外模式、指令变化。完整方法（Exp3）在全部 84 个场景中实现 ≥98% 存活率，而无风基线（Exp1）在 45/84 场景中失败。

## 附录：实验结果总表

| 实验 | 描述 | L5 平均存活率 | L5 跟踪误差 | 失败场景数 |
|------|------|-------------|------------|-----------|
| Exp1 | 无风训练基线 | 5% | 1.617 | 45/84 |
| Exp2 | Push 扰动替代风模型 | 81% | 1.180 | 16/84 |
| **Exp3** | **完整方法（风+课程+奖励）** | **100%** | **0.254** | **0/84** |
| Exp4 | 消融：无课程学习 | 48% | 0.910 | 18/84 |
| Exp5 | 消融：无风奖励 | 99% | 0.272 | 0/84 |

详细结果见 `test_results/` 和 `g1_wind_doc/g1_wind_test.md`。
